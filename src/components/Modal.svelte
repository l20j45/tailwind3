...
{#if isOpen && boot}
    <div 
        ...
        tabindex="-1"
    >
        <div class="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-white rounded-3xl shadow-2xl animate-slide-up">
            <!-- Botón cerrar -->
            <button
            ...
            <div class="p-8">
                ...
                <div class="mb-6 relative overflow-hidden bg-white bg-opacity-30 rounded-2xl">
                    ...
                    {#if boot?.new === "yes"}
                        <div class="absolute top-4 right-4">
                            <span class="px-3 py-1.5 text-sm font-medium text-center text-green-400 uppercase border border-green-400 rounded-full bg-black bg-opacity-50">
                                Nuevo
                            </span>
                        </div>
                    {/if}
                ...
                <div>
                    ...
                    {#if boot.categoria}
                        <div class="mb-6">
                            <span class="inline-block px-4 py-2 text-sm font-medium text-black bg-gray-200 rounded-full">
                                {boot.categoria}
                            </span>
                        </div>
                    {/if}
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {#each boot.Beneficios as beneficio}
                                <div class="flex items-start text-black text-opacity-80">
                                    <svg class="mr-3 mt-1 text-green-400 flex-shrink-0" width="20" height="20" viewBox="0 0 16 16" fill="none">
